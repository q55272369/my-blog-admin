...
const Icons = {
  ...
  Loader: () => <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="#007aff" strokeWidth="3" strokeLinecap="round" className="spinning"><circle cx="12" cy="12" r="10" strokeOpacity="0.25"></circle><path d="M12 2a10 10 0 0 1 10 10"></path></svg>
};
...
const NotionViewer = ({ blocks }) => (
  <div style={{color:'#e1e1e3', fontSize:'15px', lineHeight:'1.8'}}>
    {blocks?.map((b, i) => {
      ...
      if(b.type==='image') return <img key={i} src={d.external?.url || d.file?.url} style={{width:'100%', borderRadius:'10px', margin:'15px 0'}} alt="" />;
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    s.innerHTML = `
      ...
      .card-item { position: relative; background: #424242; border-radius: 12px; margin-bottom: 12px; display: flex !important; flex-direction: row !important; align-items: stretch; overflow: hidden; cursor: pointer; transition: 0.3s; border: 1px solid transparent; }
      ...
      .loading-overlay { position: fixed; top: 20px; right: 20px; background: #202024; border: 1px solid #333; padding: 12px 20px; border-radius: 10px; display: flex; align-items: center; gap: 12px; z-index: 2000; box-shadow: 0 4px 15px rgba(0,0,0,0.5); }
    `;
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#303030', padding: '40px 20px' }}>
      {/* 🟢 交互增强：全局 Loading 提示 */}
      {loading && <div className="loading-overlay"><Icons.Loader /> <span style={{fontSize:'13px', fontWeight:'bold'}}>正在同步云端数据...</span></div>}
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        {view === 'list' ? (
          <main>
            ...
            <div style={s.toolbar}>
                <button onClick={() => {setViewMode('folder'); setSelectedFolder(null);}} style={viewMode==='folder'?s.iconOn:s.iconBtn}><Icons.FolderMode /></button>
                <button onClick={() => setViewMode('covered')} style={viewMode==='covered'?s.iconOn:s.iconBtn}><Icons.CoverMode /></button>
                <button onClick={() => setViewMode('text')} style={viewMode==='text'?s.iconOn:s.iconBtn}><Icons.TextMode /></button>
                <button onClick={() => setViewMode('gallery')} style={viewMode==='gallery'?s.iconOn:s.iconBtn}><Icons.GridMode /></button>
            </div>
          ...
          <main style={s.panel}><div style={{marginBottom:'20px'}}><label style={s.lab}>标题 <span className="required-star">*</span></label><input value={form.title} onChange={e=>setForm({...form, title:e.target.value})} /></div>
            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'20px', marginBottom:'20px'}}>
              <div><label style={s.lab}>分类 <span className="required-star">*</span></label><input list="cats" value={form.category} onChange={e=>setForm({...form, category:e.target.value})} /><datalist id="cats">{options.categories.map(o=><option key={o} value={o}/>)}</datalist></div>
              <div><label style={s.lab}>发布日期 <span className="required-star">*</span></label><input type="date" value={form.date} onChange={e=>setForm({...form, date:e.target.value})} /></div>
            </div>
            <div style={{marginBottom:'20px'}}><label style={s.lab}>标签</label><input value={form.tags} onChange={e=>setForm({...form, tags:e.target.value})} /><div style={{marginTop:'10px', display:'flex', flexWrap:'wrap'}}>{displayTags.map(t => <span key={t} className="tag-chip" onClick={()=>{const cur=form.tags.split(',').filter(Boolean); if(!cur.includes(t)) setForm({...form, tags:[...cur,t].join(',')})}}>{t}<div style={{fontSize:'10px', marginLeft:'5px', color:'red'}} onClick={(e)=>{e.stopPropagation(); if(confirm('删掉此标签？')){setLoading(true); fetch('/api/tags?name='+t,{method:'DELETE'}).then(()=>fetchPosts())}}}>×</div></span>)}{options.tags.length > 12 && <span onClick={()=>setShowAllTags(!showAllTags)} style={{fontSize:'12px', color:'#007aff', cursor:'pointer', fontWeight:'bold', marginLeft:'5px'}}>{showAllTags ? '收起' : `...`}</span>}</div></div>
            ...
            <textarea ref={textAreaRef} style={{height:'500px', borderRadius:'0 0 10px 10px', background:'#18181c', color:'#fff', padding:'15px'}} value={form.content} onChange={e=>setForm({...form, content:e.target.value})} placeholder="写正文..." />
