...
const NotionView = ({ blocks }) => (
  <div style={{color:'#e1e1e3', fontSize:'15px', lineHeight:'1.8'}}>
    {blocks?.map((b, i) => {
      ...
      if(b.type==='callout') return <div key={i} style={{background:'#2d2d30', padding:'16px 24px', borderRadius:'10px', border:'1px solid #3e3e42', display:'flex', gap:'12px', margin:'1em 0'}}><div style={{fontSize:'1.3em'}}>{b.callout.icon?.emoji || '🔒'}</div><div style={{flex:1}}><div style={{fontWeight:'bold', color:'#007aff', marginBottom:'4px'}}>{t}</div><div style={{fontSize:'0.9em', opacity:0.6}}>[ 加密内容 ]</div></div></div>;
      return null;
    })}
  </div>
);
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            {view === 'list' && <><button onClick={() => setIsSearchOpen(!isSearchOpen)} style={s.iconBtn} className="btn-ia"><Icons.Search /></button><button onClick={() => { setForm({ title: '', slug: 'p-' + Date.now().toString(36), excerpt: '', content: '', category: '', tags: '', cover: '', status: 'Published', type: activeTab, date: new Date().toISOString().split('T')[0] }); setCurrentId(null); setView('edit'); }} style={s.btnBlue} className="btn-ia">发布新内容</button></>}
            {view === 'edit' && <button onClick={() => setView('list')} style={s.btnGray} className="btn-ia">返回</button>}
          </div>
        </header>
        ...
        {view === 'list' ? (
            ...
            <div style={viewMode === 'gallery' || viewMode === 'folder' ? {display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(200px, 1fr))', gap:'15px'} : {}}>
              ...
              {viewMode !== 'folder' && filteredPosts.map(p => (
                <div key={p.id} onClick={()=>handlePreview(p)} className="card-item" style={viewMode === 'text' ? s.cardText : viewMode === 'gallery' ? s.cardGallery : {}}>
                  ...
                  {viewMode === 'text' && <div style={{flex:1, display:'flex', alignItems:'center'}}><div style={{flex:1}}>{p.title}</div><div style={s.meta}>{p.category} · {p.date}</div></div>}
                  {viewMode === 'gallery' && <>
                  ...
                  <div className="drawer-zone"><div onClick={(e)=>{e.stopPropagation(); handleEdit(p)}} style={s.drEdit} className="dr-btn"><Icons.Edit /></div><div onClick={(e)=>{e.stopPropagation(); if(confirm('彻底删除？')){fetch('/api/post?id='+p.id,{method:'DELETE'}).then(()=>fetchPosts())}}} style={s.drDel} className="dr-btn"><Icons.Trash /></div></div>
                </div>
              ))}
            </div>
          </main>
          ...
          <main style={s.panel}><div style={{marginBottom:'20px'}}><label style={s.lab}>标题 *</label><input value={form.title} onChange={e=>setForm({...form, title:e.target.value})} /></div>
            ...
            <div style={s.toolBox}><button onClick={()=>window.open("https://x1file.top/home")} style={s.btnGrayFull} className="btn-ia">🎬 打开网盘获取素材</button><textarea style={{height:'120px', background:'#18181c'}} placeholder="直链转换..." value={rawLinks} onChange={e=>setRawLinks(e.target.value)} /><button onClick={()=>{const lines=rawLinks.split('\n'); const final=[]; for(let i=0; i<lines.length; i++){const m=lines[i].match(/https?:\/\/[^\s]+/); if(m) final.push(`![](${m[0]})`);} setMdLinks(final.join('\n'))}} style={s.btnBlueFull} className="btn-ia">立即转换</button>{mdLinks && <button onClick={()=>{navigator.clipboard.writeText(mdLinks); alert('已复制')}} style={s.btnGrayFull} className="btn-ia">复制全部结果</button>}</div>
            ...
            <textarea ref={textAreaRef} style={{height:'500px', borderRadius:'0 0 10px 10px', background:'#18181c'}} value={form.content} onChange={e=>setForm({...form, content:e.target.value})} placeholder="写正文..." />
            <button onClick={()=>{setLoading(true); fetch('/api/post',{method:'POST', body:JSON.stringify({...form, id:currentId})}).then(()=>{setView('list'); fetchPosts();})}} disabled={!isFormValid} style={isFormValid?s.btnPublish:s.btnDisabled} className="btn-ia">确认发布</button>
          </main>
        )}
...
const s = {
  ...
  cardText: { display:'flex', alignItems:'center', padding:'16px 20px', borderRadius:'0', borderBottom:'1px solid #333' },
  cardGallery: { display:'flex', flexDirection:'column', height:'auto' },
  ...
  drEdit: { background:'#007aff' }, drDel: { background:'#ff4d4f' },
